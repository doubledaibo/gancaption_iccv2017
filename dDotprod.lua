...
local decoder = require 'decoderDotprod'
...
function layer:__init(opt)
	...
	self.decoder_cap_input = torch.Tensor()
	self.img_proj = nn.Sequential()
	self.img_proj:add(nn.Linear(self.cnn_output_size, self.input_encoding_size))
	self.img_proj:add(nn.ReLU(true))
end
...
function layer:getModulesList()
	return {self.core, self.lookup_table, self.decoder, self.img_proj}
end
...
function layer:parameters()
	...
	local p4, g4 = self.img_proj:parameters()
	local params = {}
...
function layer:updateOutput(input)
	...
	local img = self.img_proj:forward(input[1])
	self.output:resize(self.batch_size, 1)
	self.decoder_cap_input:resize(self.batch_size, self.rnn_size):zero()
	...
	for b = 1, self.batch_size do
		self.decoder_cap_input[b] = self.state[self.last_token[b]][self.num_state][b]
	end
	self.decoder_input = {img, self.decoder_cap_input}
	local decoder_out = self.decoder:forward(self.decoder_input)
	self.output[{{}, {1, 1}}] = decoder_out
	return self.output 				 
end
...
function layer:updateGradInput(input, gradOutput)
	...
	local dcap
	dimg, dcap = unpack(self.decoder:backward(self.decoder_input, gradOutput))
	for t = self.tmax, 1, -1 do
		for b = 1, self.batch_size do
			if self.last_token[b] == t then
				dstate[t][self.num_state][b] = dcap[b]
			end
		end
	...
	self.gradInput = self.img_proj:backward(input[1], dimg)
